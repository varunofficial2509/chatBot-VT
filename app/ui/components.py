...
# Simple Icons slugs (https://simpleicons.org), verified to resolve via the
# public CDN. Anything not listed here renders as clean mono text instead of
...
def render_nav_header(active: str, name: str = "") -> None:
    handle = " ".join(name.split()[:2]).lower() if name else ""
    links_html = "".join(
        f'<a class="{"active" if active == matches else ""}" href="{href}">/{label}</a>'
        for label, href, matches in NAV_ITEMS
    )
    st.html(
        f"""
        <div class="vt-nav">
            <a class="vt-nav-brand" href="/home"><span class="accent">$</span> {handle}.</a>
...
def render_skills(categories: dict[str, list[str]]) -> None:
    ...
    for category, skills in categories.items():
        items_html = ""
        for skill in skills:
            slug = SKILL_ICONS.get(skill)
            marker = (
                f'<img src="https://cdn.simpleicons.org/{slug}/F2F2F2" alt="" loading="lazy" />'
                if slug
                else '<span class="vt-skill-dot"></span>'
            )
            items_html += f'<div class="vt-skill-item">{marker}<span>{skill}</span></div>'
        columns_html += (
...
def render_experience_item(job: dict, technologies: list[str] | None = None) -> None:
    product = f" · {job['product']}" if job.get("product") else ""
    tech_html = (
        f'<div class="vt-exp-tech">{" · ".join(technologies)}</div>' if technologies else ""
    )
    st.html(
        f"""
        <div class="vt-exp">
            <div class="vt-exp-period mono">{job.get("period", "")}</div>
            <div class="vt-exp-role">{job.get("role", "")}</div>
            <div class="vt-exp-company">{job.get("company", "")}{product}</div>
            <div class="vt-exp-desc">{job.get("description", "")}</div>
            {tech_html}
        </div>
        """
    )
